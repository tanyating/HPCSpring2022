...
#pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid)
  ...
  #pragma omp sections nowait
    ...
    #pragma omp section
      ...
      for (i=0; i<N; i++)
        a[i] = i * DELTA;
      omp_unset_lock(&locka); // we can unlock a here since it's read-only later (release critical region a to the other section to prevent deadlock)
      omp_set_lock(&lockb);
    ...
    #pragma omp section
      ...
      for (i=0; i<N; i++)
        b[i] = i * PI;
      omp_unset_lock(&lockb); // we can unlock b here since it's read-only later (release critical region b to the other section to prevent deadlock)
      omp_set_lock(&locka);
